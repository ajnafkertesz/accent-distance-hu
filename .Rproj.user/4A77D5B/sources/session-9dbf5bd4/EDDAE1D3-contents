...
HUN_audios <- audios[grepl("_HUN.wav$", audios)]
other_audios <- audios[!grepl("_HUN.wav$", audios)]

RU = "RU"
US = "US"

# select 30 random words from this list
random_HUN_audios <- sample(HUN_audios, 30)

# extract all word stems from random_HUN_audios
prefixes <- sub("_.*", "", random_HUN_audios)
...
instructions6 <- trial_instructions(
  pages = c('<p> Összesen 30 triádot fogsz hallani.</p>
...
trial1 <- trial_instructions(
  pages = c("1. Triád: Kattints a \"továbbra\" a kezdéshez."),
  show_clickable_nav = TRUE,
  button_label_previous = "Előző",
  button_label_next = "Tovább",
  post_trial_gap = 250
...
random_other <- random_other_audios[
  grepl(paste0("^", phrase, "_"), random_other_audios)&
    (grepl(paste0(RU), random_other_audios)|
    grepl(paste0(US), random_other_audios))
  ]
random_other <- sample(random_other)

# filter out these phrases 
random_HUN_audios <- random_HUN_audios[!grepl(paste0("^", phrase, "_"), random_HUN_audios)]
random_other_audios <- random_other_audios[!grepl(paste0("^", phrase, "_"), random_other_audios)]

# play HU prompt
us_prompt1 <- trial_audio_button_response(
  stimulus = insert_resource(random_HUN),
  choices = c("Tovább"),
